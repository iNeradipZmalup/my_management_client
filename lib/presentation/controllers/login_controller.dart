...
class LoginController extends GetxController {
  ...
  Future<LoginState> executeRequest(String email, String password) async {
    state = state.copyWith(statusRequest: StatusRequest.loading);

    final (success, message, user) = await UserRemoteDataSource.login(
      email,
      password,
    );
...
class LoginState {
  ...
  LoginState({required this.statusRequest, required this.message});

  LoginState copyWith({StatusRequest? statusRequest, String? message}) {
    return LoginState(
